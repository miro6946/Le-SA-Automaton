...
(function($){
	$.fn.tooltip = function(options) {
		var defaults = {
			...
			duration : 300,   //The number of milliseconds after moving the mouse cusor before removing the tooltip.
			xOffset : 25,     //X offset will allow the tooltip to appear offset by x pixels.
			yOffset : 25,     //Y offset will allow the tooltip to appear offset by y pixels.
			opacity : 0,      //0 is completely opaque and 100 completely transparent
			fadeDuration: 200 //[toxi20090112] added fade duration in millis (default = "normal")
		};
		...
		return this.each(function(index) {
			...
			$this.mouseover( function(e) {
				if (typeof $this.data('title') == 'undefined' && (typeof $this.attr('title') == 'undefined' || $this.attr('title').replace(/\s/, '') == '')) {
					return false;
				}
				...
				$tooltip.data('title', $this.data('title'));
//				ie에서 select box가 닫히는 문제로 주석 처리 2009-10-22 juriel
				$this.attr('title', '');
...
//				$tooltip.data('alt', '');
				...
				$tooltip.empty().hide().removeClass($tooltip.attr('class')).css({width: 0, height: 0});
			});
